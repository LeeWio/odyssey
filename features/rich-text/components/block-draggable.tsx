  ...
  return (
    ...
    >
      {!isInTable && (
        <Gutter>
          <div
            className={cn("slate-blockToolbarWrapper", "flex h-[1.5em] ", isInColumn && "h-4")}
            style={{ marginTop: dragButtonTop }}
          >
            <div
              className={cn(
                "slate-blockToolbar pointer-events-auto flex items-center justify-center ",
                isInColumn && "mr-1.5"
              )}
            >
              <DragHandle
...
function Gutter({ children, className, ...props }: React.ComponentProps<"div">) {
  ...
  return (
    <div
      ...
      className={cn(
        "slate-gutterLeft",
        "absolute top-1 z-50 flex h-full -translate-x-full cursor-text",
    ...
    <Tooltip delay={0}>
      <Tooltip.Trigger>
        <div
          ...
          role="button"
          className={cn(
            buttonVariants({ isIconOnly: true, size: "sm", variant: "ghost" })
          )}
...
const DropLine = memo(function DropLine({ className, ...props }: React.ComponentProps<"div">) {
