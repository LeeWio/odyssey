...
- 在 401 响应时尝试续期，并仅在当前会话无法续期时清除本地鉴权状态。
...
- 同一 Redux store、同一组凭证的并发 401 共用一次刷新请求；不同 store 的续期互相隔离。
- 刷新成功后，用新 access token 重试原请求一次；稍后返回的旧 token 401 可复用最近一次刷新结果。
- 退出登录或切换账号后，旧请求和刷新结果不得覆盖或清除新会话，也不得以新账号重放旧请求。
- 刷新接口明确拒绝凭证（401/403），或成功响应中的凭证格式无效时，清除对应会话及 API 缓存。
- 刷新遇到网络错误、超时或服务临时故障时，保留凭证并返回错误，允许后续请求再次尝试。

以上行为由 `lib/api/base-api.test.ts` 通过独立 store 和可控响应顺序验证。
...
- Redux Provider 使用与服务端一致的初始状态快照完成 hydration，再向组件呈现浏览器中持久化的登录状态和偏好，避免首次渲染不一致。
- access token 由 auth slice 管理，并通过请求头注入。
- 401 响应先经过统一续期处理；只能清除仍与该请求匹配、且无法续期的会话。
