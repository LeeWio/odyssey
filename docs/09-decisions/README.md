# ADR Index

Use this directory for architecture decision records.

## Suggested Format

- one decision per file
- include context, decision, alternatives, and consequences
- name files with stable numeric prefixes

## Current Records

- [ADR-0001: Product Direction](./ADR-0001-product-direction.md)
- [ADR-0002: Documentation Structure](./ADR-0002-document-structure.md)
- [ADR-0003: Module Lifecycle](./ADR-0003-module-lifecycle.md)
- [ADR-0004: Animation Standard](./ADR-0004-animation-standard.md)
- [ADR-0005: HeroUI Baseline](./ADR-0005-heroui-baseline.md)
- [ADR-0006: Comment Thread Model](./ADR-0006-comment-thread-model.md)
