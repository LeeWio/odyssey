  ...
  return (
    ...
    >
      {profileItems.map((item) => (
        <ItemCard key={item.title} className="flex-1">
          <ItemCard.Icon>{getIcon(item.icon)}</ItemCard.Icon>
          <ItemCard.Content>
...
function RecentGamePurchaseCards() {
  return (
    <motion.div className="flex w-full flex-1 flex-col gap-4 md:col-span-6 lg:col-span-3">
      {recentGames.map((game, idx) => (
        ...
        >
          ...
          <ItemCard.Content>
            <ItemCard.Title className="font-bold">{game.title}</ItemCard.Title>
            {game.description && <ItemCard.Description>{game.description}</ItemCard.Description>}
          </ItemCard.Content>
          <ItemCard.Action>
...
function NewGamesCarouselSection() {
  return (
    <motion.div variants={itemVariants} className="flex w-full flex-col gap-4">
      ...
      <div className="w-full">
        <Carousel opts={{ align: "start" }}>
          ...
          <Carousel.Previous aria-label="Previous game" />
          <Carousel.Next aria-label="Next game" />
        </Carousel>
      </div>
    </motion.div>
  );
      ...
      >
        ...
        <div className="z-10 flex items-center gap-6">
          ...
          <div className="flex items-center gap-2">
            <Button isIconOnly variant="primary" size="lg" aria-label="Play game">
              <Icon icon="gravity-ui:play-fill" />
            </Button>
            <Button isIconOnly size="lg" variant="tertiary" aria-label="Close game">
  ...
  return (
    <motion.div className="flex h-full w-full flex-col gap-4 lg:col-span-1">
      <motion.div variants={itemVariants} className="flex items-end justify-between">
        ...
        <Link href="#" className="text-muted text-xs" aria-label="See all statistics">
          →
        </Link>
      </motion.div>
    ...
    <AnimatePresence mode="wait">
      <motion.div
        ...
        className={`pointer-events-none absolute inset-y-0 right-0 z-0 ${
          content.isCutout ? content.imageClassName : "w-3/5"
        }`}
        style={
          !content.isCutout
            ? {
      ...
      >
        <Image
          src={content.image}
          alt={content.title}
          fill={!content.isCutout}
          width={content.isCutout ? 800 : undefined}
          height={content.isCutout ? 800 : undefined}
          className={
            content.isCutout
  ...
  return (
    <main className="flex min-h-screen w-full flex-col overflow-x-hidden">
      ...
      >
        ...
        >
          <HeroImage activeTab={activeTab} content={currentContent} />
          <div className="relative z-10 flex w-full flex-col sm:w-[65%]">
            <Card.Header className="pb-6">
              ...
              >
                {tabs.map((tab) => (
                  <Segment.Item key={tab.id} className="w-auto" id={tab.id} style={{ gap: 0 }} aria-label={tab.label}>
                    {({ isSelected }) => (
                      <>
                        <Icon icon={tab.icon} />
                        <span
      ...
      >
        <motion.div className="flex w-full flex-col gap-6 lg:col-span-2">
          <NewGamesCarouselSection />
          <LastDownloadsSection />
        </motion.div>
        <StatisticsSection />
      </MotionSurface>
    </main>
