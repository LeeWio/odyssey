...
export default function Home() {
  ...
  return (
    <main className="flex min-h-screen flex-col items-center justify-center bg-[#0a0a0b] p-6 gap-12">
      {/* Time & Weather Widget (Restored) */}
      <Card className="w-full max-w-[380px] bg-[#1c1c1e] border-none shadow-none rounded-[3.5rem] p-10 flex flex-col items-center gap-6 overflow-hidden">
        ...
        <Card.Content className="p-0 py-4 flex justify-center w-full">
          <Typography
            render={({ children, ...domProps }) => <div {...domProps}>{children}</div>}
            className="text-white text-[7.5rem] font-bold tracking-tighter leading-[0.8] flex items-center tabular-nums"
          >
            <AnimatedNumber value={parseInt(hours)} format={{ minimumIntegerDigits: 2 }} />
        ...
        <Card.Footer className="p-0 flex justify-center items-center gap-2.5">
          ...
          <Typography className="text-white text-2xl font-bold tracking-tight">10-30°</Typography>
        </Card.Footer>
      </Card>

      {/* Rotating Music Avatar Demo */}
      <div ref={avatarContainerRef} className="flex flex-col items-center gap-4">
        <Avatar className="music-disk w-24 h-24 rounded-full ring-8 ring-white/5 shadow-2xl">
          <Avatar.Image
            alt="Small Avatar"
        ...
        <Typography className="text-white/40 text-sm font-medium uppercase tracking-widest">
          Now Playing
        </Typography>
      </div>
    </main>
