...
export default function FileTestPage() {
  ...
  const handleRetry = async (fileItem: UploadFile) => {
    ...
    // 注意：需要原本的 File 对象才能重新上传，隔离测试中可以直接提示用户重新拖入
  };
  ...
  return (
    <div className="animate-fade-in mx-auto flex w-full max-w-2xl flex-col gap-8 px-6 py-8">
      {/* 头部标题区 */}
      <div className="border-border flex flex-col gap-2 border-b pb-4">
        <h1 className="text-foreground text-2xl font-bold tracking-tight">文件与素材管理</h1>
        <p className="text-muted mt-1 text-sm">
          完全按照 `@heroui-pro/react` 官方 API 和示例代码编写的 `DropZone` 上传验证测试。
        </p>
      </div>
            ...
            {files.map((file) => {
              ...
              return (
                <DropZone.FileItem key={file.id} status={file.status}>
                  ...
                  {/* 自定义辅助行动点 (复制外链 / 新窗口预览) - 紧跟在删除按钮左侧，保持视觉纯粹 */}
