...
for (const width of [390, 1440]) {
  test(`home flight paths map renders at ${width}px`, async ({ page }) => {
    ...
    await expect(
      section.getByText("A record of the cities I've visited, and the moments that made me stay.")
    ).toBeVisible();
    await expect(section.getByText("Flight operations")).toBeVisible();
    await expect(section.getByText("SFO → HND")).toBeVisible();
...
test("timeline, popup and year selection stay in sync", async ({ page }) => {
  ...
  const entry = page.getByRole("button", { name: "Read memory from Shinjuku, Tokyo" });
  await entry.click();
  await expect(entry).toHaveAttribute("aria-pressed", "true");
  await expect(page.getByLabel("Selected travel memory")).toContainText("Shinjuku, Tokyo");
  await expect(page.locator('[data-slot="map-popup"]')).toContainText("Shinjuku, Tokyo");
